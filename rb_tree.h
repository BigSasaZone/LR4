...
public:
    ...
    void Stitch();
    DynamicArray<T> GoForward() const;
    std::string Read(const std::string& template_str) const;
    Set<T, Comparator> GetSubTree(const T& key) const;
    size_t Count(Node_* node) const;
    bool IsSubTree(const Set<T, Comparator>& other) const;
    bool IsSubTree(Node_* node, Node_* other) const;
    template<class Unary>
    Set<T, Comparator> Map(const Unary& unary) const;
    template<class Predicate>
    Set<T, Comparator> Where(const Predicate& predicate) const;
    template<class Binary, class U>
    U Reduce(const Binary& binary,U u) const;
};

template <typename T, class Comparator>
const T& Set<T, Comparator>::Iterator::operator*()const{
    return ptr_->data;
}

template <typename T, class Comparator>
const bool Set<T, Comparator>::BLACK_ = true;

template <typename T, class Comparator>
const bool Set<T, Comparator>::RED_ = false;

template <typename T, class Comparator>
const size_t Set<T, Comparator>::LEFT_ = 0;

template <typename T, class Comparator>
const size_t Set<T, Comparator>::RIGHT_ = 1;

template <typename T, class Comparator>
bool Set<T, Comparator>::GetColour(const Node_* node)const{
    if(node == nullptr){
        return BLACK_;
    }
    return node->colour;
...
template <typename T, class Comparator>
void Set<T, Comparator>::Connect(Node_* child, Node_* parent) const{
    if(child != nullptr){
        child->parent = parent;
    }
}
...
template <typename T, class Comparator>
typename Set<T, Comparator>::Node_* Set<T, Comparator>::LeftTurn(Node_* node){
...
template <typename T, class Comparator>
typename Set<T, Comparator>::Node_* Set<T, Comparator>::RightTurn(Node_* node){
...
template <typename T, class Comparator>
void Set<T, Comparator>::Insert(const T& value) {
    if (root_ == nullptr) {
...
template <typename T, class Comparator>
Set<T, Comparator>::Set(Comparator cmp) : root_(nullptr), size_(0), cmp_(cmp) {}

template <typename T, class Comparator>
typename Set<T, Comparator>::Node_* Set<T, Comparator>::FindNode(const T& value) const {
...
template <typename T, class Comparator>
typename Set<T, Comparator>::Node_* Set<T, Comparator>::Copy(Node_* node) const{
    if(node == nullptr){
        return nullptr;
    }
...
template <typename T, class Comparator>
Set<T, Comparator>::Set(const Set& other) : root_(Copy(other.root_)), size_(other.size_), cmp_(other.cmp_) {
    Stitch();
}
template <typename T, class Comparator>
void Set<T, Comparator>::Delete(Node_* node){
    if(node == nullptr){
        return;
    }
...
template <typename T, class Comparator>
Set<T, Comparator>::Iterator::Iterator(Node_* node): ptr_(node){}
...
template <typename T, class Comparator>
bool Set<T, Comparator>::operator==(const Set& other) const{
...
template <typename T, class Comparator>
DynamicArray<T> Set<T, Comparator>::GoForward()const{
    auto iter = begin();
    DynamicArray<T> data;
    while(iter != end()){
        data.Append(*iter);
...
template<typename T, class Comparator>
void Set<T, Comparator>::InOrder(std::string& result, char item1, char item2, char item3, Node_* node, char first, char second, char third) const{
    if(node == nullptr) {
        return;
    }
    char close;
...
template<typename T, class Comparator>
std::string Set<T, Comparator>::Read(const std::string& template_str) const{
...
template<typename T, class Comparator>
size_t Set<T, Comparator>::Count(Node_* node) const{
    if(node == nullptr){
        return 0;
    }
...
template<typename T, class Comparator>
Set<T, Comparator> Set<T, Comparator>::GetSubTree(const T& key) const{
    Node_* sub_root = Find(key).ptr_;
    Node_* node = Copy(sub_root);
    if(GetColour(node) == RED_){
        node->colour = BLACK_;
    }
    Set<T, Comparator> sub_tree(cmp_);
...
template<typename T, class Comparator>
bool Set<T, Comparator>::IsSubTree(const Set<T, Comparator>& other) const{
...
template<typename T, class Comparator>
bool Set<T, Comparator>::IsSubTree(Node_* node, Node_* other) const{
...
template<typename T, class Comparator>
template<class Unary>
Set<T, Comparator> Set<T,Comparator>::Map(const Unary& unary)const{
    Set map_set;
    auto iter = begin();
    while(iter != end()){
        map_set.Insert(unary(*iter));
        ++iter;
    }
    return map_set;
}
...
template<typename T, class Comparator>
template<class Predicate>
Set<T, Comparator> Set<T,Comparator>::Where(const Predicate& predicate)const{
    Set selected_set;
    auto iter = begin();
    while(iter != end()){
        if(predicate(*iter)){
            selected_set.Insert(*iter);
        }
        ++iter;
    }
    return selected_set;
...
template<typename T, class Comparator>
template<class Binary, class U>
U Set<T,Comparator>::Reduce(const Binary& binary,U u)const{
    for(const auto& elem: *this){
        u = binary(elem, u);
    }
    return u;
